...
class Thread_DownloadUV(QtCore.QThread):
    ...
    def run(self):
        ...
        if link != '':
            self.The_link = link.replace('&', '"&"')                                                                      # Replace with ""........
            self.MainCode.label_Pcomplete_UTV.setText("................... Downloading Started ...................")      # printing in status bar ....
        else:
        ...
        if self.MainCode.radioButton_UTV_AudioOnly.isChecked():
            ...
            if self.Video_folder != '':
                ...
                self.MainCode.progressBar_YT_V.setValue(0)                                                               # Setting progress bar value to 0 ...
    ...
    def Format_Selection(self):
        if self.MainCode.radioButton_UTV_AudioOnly.isChecked():
            self.VideoFormat = 'bestaudio/best'                                                     # Get best Audio in quality......................   
        else:
            if self.Format_Requested == 0:
                self.VideoFormat = 'bestvideo*+bestaudio/best'                                      # Get best Video in quality...................... 
            if self.Format_Requested == 1:
                self.VideoFormat = 'bestvideo[height<=1080]+bestaudio/best[height<=1080]'           # 1080p ................
            if self.Format_Requested == 2:
                self.VideoFormat = 'bestvideo[height<=720]+bestaudio/best[height<=720]'           # 720p ................
            if self.Format_Requested == 3:
                self.VideoFormat = 'bestvideo[height<=480]+bestaudio/best[height<=480]'           # 480p ................
            if self.Format_Requested == 4:
                self.VideoFormat = 'bestvideo[height<=360]+bestaudio/best[height<=360]'           # 360p ................
            if self.Format_Requested == 5:
                self.VideoFormat = 'bestvideo[height<=240]+bestaudio/best[height<=240]'           # 240p ................
            if self.Format_Requested == 6:
                self.VideoFormat = 'bestvideo[height<=144]+bestaudio/best[height<=144]'           # 144p ................
    ...
    def Downloader(self):
        ...
        if self.MainCode.radioButton_UTV_subs.isChecked():
            ...
            print(f"Selected Sub Language : {self.Requested_Sub_lang}")                            # Selected Subtitle language .........

        if not self.MainCode.radioButton_YTV_subM.isChecked():                                     # Downloading Audio in quality sub titles ....... 
            print("Download sub + Audio in quality")                                               
            if self.VideoFormat != 'bestaudio/best':                                               # best Audio in quality download .....
    ...
    def My_Progress_hook(self, d):
        ...
        if d['status'] == 'finished':
            ...
            Speed = d['_speed_str']                          # getting speed from Json.......    
            ...
            TimeRemaining = d['_eta_str']                    # Getting speed from Json.......
            self.TimeRemains.emit(TimeRemaining)             # Send through signal......
